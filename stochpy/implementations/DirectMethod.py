...
class DirectMethod(StochPySSA_Shared):
    ...
    def ExecuteWholeCellCycle(self, settings, IsStatusBar=False, trajectory=1, task_id=''):
        ...
        self._IsInitial = True
        self.settings = settings
        self.last_phase_started = False
        # does not have to start at zero if we perform sequential simulations, although in our case
        ...
        # ensure that the first run updates all species
        self.species_to_update = [s for s in range(self.n_species)]
        self.Propensities()
        ...
        if not self.sim_t:
            ...
            self.SpeciesSelection()
            self.RateSelection()
            # April 15, moved into here, because otherwise each new cell division cycle
            # starts with a time event, if specified
            self.SetEvents()
            if not settings.IsOnlyLastTimepoint:
                self.Initial_Conditions()
        ...
        while (self.sim_t < settings.endtime) and (self.timestep < settings.timesteps):
            ...
            if self.sim_t < settings.endtime:
                if not self._IsPerformEvent:
                    self.species_to_update = self.parse.reaction_affects[
                        self.reaction_index]  # Determine vars to update
                else:
                    self.species_to_update = [s for s in range(self.n_species)]
